...
class SWAGInference(object):
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            self.theta_swa[name] = ((self.theta_swa[name] * self.model_counter) + param) / (self.model_counter + 1)
            self.theta_square_bar[name] = ((self.theta_square_bar[name] * self.model_counter) + (param * param)) / (self.model_counter + 1)
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            self.D.append({name: param - self.theta_swa[name] for name, param in current_params.items()})
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        # self.theta_square_bar = self._create_weight_copy()
        # self.theta_bar = self.network.layer0[0].weight
        # self.theta_square_bar = self.network.layer0[0].weight ** 2
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            # Initialize a list to store predictions for the current model sample
            ...
            for (batch_xs, ) in loader:  # Assuming loader yields (data, label) pairs or (data, label1, label2) triples etc.
                pred_ys = self.network(batch_xs)
                # raise Exception('test')
                current_model_predictions.append(torch.softmax(pred_ys, dim=-1))
            # Stack predictions for current model sample and add to per_model_sample_predictions
            per_model_sample_predictions.append(torch.cat(current_model_predictions, dim=0))
        ...
        bma_probabilities = torch.stack(per_model_sample_predictions, dim=0)
        bma_probabilities = torch.mean(bma_probabilities, dim=0)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_std = self.theta_square_bar[name] - (self.theta_swa[name] ** 2)
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                sampled_param = current_mean \
                    + 1 / math.sqrt(2) * torch.sqrt(torch.clamp(current_std, 0, None)) * z_1
                temp_D = [temp[name] for temp in self.D]
                for x, z in zip(temp_D, z_2):
                    sampled_param += 1 / math.sqrt(2 * (self.deviation_matrix_max_rank - 1)) * x * z
